...
// Avoid PoCL builtin overload issues by using a plain positive-part macro.
#define POSPART(x) (((x) > (REAL)0) ? (x) : (REAL)0)
...
__kernel void adve_VX_im(__global PARA_DATA_SIMP *para, __global REAL *var, __global  int *BINDEX) {
  ...
  REAL dt = para->mytime.dt;
	...
	if (i>0 && i<imax && j>0 && j<jmax+1 && k>0 && k<kmax+1) {
				...
				aw[IX(i, j, k)] = POSPART(Fw);
				ae[IX(i, j, k)] = POSPART(-Fe);
				as[IX(i, j, k)] = POSPART(Fs);
				an[IX(i, j, k)] = POSPART(-Fn);
				ab[IX(i, j, k)] = POSPART(Fb);
				af[IX(i, j, k)] = POSPART(-Ff);
				ap0[IX(i, j, k)] = Dx * Dy * Dz / dt;
				b[IX(i, j, k)] = psi0[IX(i, j, k)] * ap0[IX(i, j, k)];
...
int physics_v1_in_bounds(REAL x, REAL y, REAL z, __global REAL *b) {
  return x >= b[0] && x <= b[1] && y >= b[2] && y <= b[3] && z >= b[4] && z <= b[5];
}

void physics_v1_drive_velocity(
  __global PARA_DATA_SIMP *para,
  __global REAL *var,
  int idx,
  int size,
  __global REAL *d,
  ...
  REAL max_speed) {
  REAL dt = para->mytime.dt;
  REAL relax = clamp(dt * max(damping, (REAL)0.0f), (REAL)0.0f, (REAL)0.5f);
  REAL speed = clamp(target_speed, (REAL)0.0f, max(max_speed, (REAL)0.01f));
  REAL dx = d[0], dy = d[1], dz = d[2];
  REAL mag = sqrt(dx * dx + dy * dy + dz * dz);
  REAL tu, tv, tw;
  REAL u, v, w;

  if (mag <= (REAL)1.0e-6f || relax <= (REAL)0.0f) return;
  dx /= mag;
  dy /= mag;
  dz /= mag;
  tu = dx * speed;
  ...
  v = var[VY * size + idx];
  w = var[VZ * size + idx];
  var[VX * size + idx] = u + relax * (tu - u);
  var[VY * size + idx] = v + relax * (tv - v);
  var[VZ * size + idx] = w + relax * (tw - w);
}

REAL physics_v1_sample_cold_face_temperature(
  __global PARA_DATA_SIMP *para,
  __global REAL *var,
  int i,
  int j,
  ...
  if (fabs(d[0]) > (REAL)0.5f) {
    if (d[0] > (REAL)0.0f) {
      for (ii = i; ii >= 1; ii--) {
        x = var[X * size + IX(ii, j, k)];
        y = var[Y * size + IX(ii, j, k)];
        z = var[Z * size + IX(ii, j, k)];
        if (physics_v1_in_bounds(x, y, z, cold)) return var[TEMP * size + IX(ii, j, k)];
      }
    }
    else {
      for (ii = i; ii <= imax; ii++) {
  ...
  else if (fabs(d[1]) > (REAL)0.5f) {
    if (d[1] > (REAL)0.0f) {
      for (ii = j; ii >= 1; ii--) {
        x = var[X * size + IX(i, ii, k)];
        y = var[Y * size + IX(i, ii, k)];
        z = var[Z * size + IX(i, ii, k)];
        if (physics_v1_in_bounds(x, y, z, cold)) return var[TEMP * size + IX(i, ii, k)];
      }
    }
    else {
      for (ii = j; ii <= jmax; ii++) {
    ...
    if (d[2] > (REAL)0.0f) {
      for (ii = k; ii >= 1; ii--) {
        x = var[X * size + IX(i, j, ii)];
        y = var[Y * size + IX(i, j, ii)];
        z = var[Z * size + IX(i, j, ii)];
        if (physics_v1_in_bounds(x, y, z, cold)) return var[TEMP * size + IX(i, j, ii)];
      }
    }
    else {
      for (ii = k; ii <= kmax; ii++) {
...
REAL physics_v1_neighbor_mean_temperature(
  __global PARA_DATA_SIMP *para,
  __global REAL *var,
  int i,
  int j,
  ...
  int IMAX = imax + 2, IJMAX = (imax + 2) * (jmax + 2);
  REAL sum = (REAL)0.0f;
  REAL count = (REAL)0.0f;
  int ni, nj, nk, nidx;

  ni = i - 1; nj = j; nk = k;
  if (ni >= 1) {
    nidx = IX(ni, nj, nk);
    if (var[FLAGP * size + nidx] < -0.5f) { sum += var[TEMP * size + nidx]; count += (REAL)1.0f; }
  }
  ni = i + 1;
  if (ni <= imax) {
    nidx = IX(ni, nj, nk);
  ...
  if (count <= (REAL)0.0f) return var[TEMP * size + IX(i, j, k)];
  return sum / count;
}

__kernel void apply_physics_v1_momentum(
  __global PARA_DATA_SIMP *para,
  __global REAL *var,
  int rack_count,
  __global REAL *rack_body_bounds,
  __global REAL *rack_cold_bounds,
  __global REAL *rack_hot_bounds,
  __global REAL *rack_dir,
  __global REAL *rack_target_speed,
  __global REAL *rack_momentum_damping,
  int extractor_count,
  __global REAL *extractor_bounds,
  __global REAL *extractor_dir,
  __global REAL *extractor_target_speed,
  __global REAL *extractor_max_speed,
  __global REAL *extractor_damping,
  int tile_count,
  __global REAL *tile_bounds,
  __global REAL *tile_loss) {
  ...
  if (i < 1 || i > imax || j < 1 || j > jmax || k < 1 || k > kmax) return;

  idx = IX(i, j, k);
  if (var[FLAGP * size + idx] >= -0.5f) return;
  ...
  for (int r = 0; r < rack_count; r++) {
    __global REAL *body = &rack_body_bounds[r * 6];
    __global REAL *cold = &rack_cold_bounds[r * 6];
    __global REAL *hot = &rack_hot_bounds[r * 6];
    if (physics_v1_in_bounds(x, y, z, body) || physics_v1_in_bounds(x, y, z, cold) || physics_v1_in_bounds(x, y, z, hot)) {
      __global REAL *d = &rack_dir[r * 3];
      physics_v1_drive_velocity(para, var, idx, size, d, rack_target_speed[r], rack_momentum_damping[r], (REAL)3.0f);
    }
  }

  for (int e = 0; e < extractor_count; e++) {
    __global REAL *b = &extractor_bounds[e * 6];
    if (physics_v1_in_bounds(x, y, z, b)) {
      __global REAL *d = &extractor_dir[e * 3];
      physics_v1_drive_velocity(
        para, var, idx, size, d, extractor_target_speed[e],
        extractor_damping[e], extractor_max_speed[e]);
    }
  }

  for (int t = 0; t < tile_count; t++) {
    __global REAL *b = &tile_bounds[t * 5];
    if (x >= b[0] && x <= b[1] && y >= b[2] && y <= b[3] && fabs(z - b[4]) <= (REAL)0.08f) {
      REAL w = var[VZ * size + idx];
      REAL factor = (REAL)1.0f + dt * (REAL)0.1f * tile_loss[t] * fabs(w);
      var[VZ * size + idx] = w / max(factor, (REAL)1.0f);
    }
  }
}

__kernel void apply_physics_v1_thermal(
  __global PARA_DATA_SIMP *para,
  __global REAL *var,
  int rack_count,
  __global REAL *rack_cold_bounds,
  __global REAL *rack_hot_bounds,
  __global REAL *rack_plume_bounds,
  __global REAL *rack_dir,
  __global REAL *rack_heat_w,
  __global REAL *rack_airflow_m3s,
  __global REAL *rack_hot_volume_m3,
  __global REAL *rack_max_delta_t,
  __global REAL *rack_thermal_damping,
  __global REAL *rack_plume_mixing,
  __global REAL *rack_source_delta_limit) {
  ...
  y = var[Y * size + idx];
  z = var[Z * size + idx];
  rho = max(para->prob.rho, (REAL)0.001f);
  cp = max(para->prob.Cp, (REAL)1.0f);
  dt = para->mytime.dt;

  for (int r = 0; r < rack_count; r++) {
    __global REAL *hot = &rack_hot_bounds[r * 6];
    __global REAL *plume = &rack_plume_bounds[r * 6];
    int in_hot = physics_v1_in_bounds(x, y, z, hot);
    int in_plume = physics_v1_in_bounds(x, y, z, plume);
    if (in_hot || in_plume) {
      __global REAL *cold = &rack_cold_bounds[r * 6];
      __global REAL *d = &rack_dir[r * 3];
      REAL airflow = max(rack_airflow_m3s[r], (REAL)0.05f);
      REAL hot_volume = max(rack_hot_volume_m3[r], (REAL)1.0e-6f);
      REAL max_delta = clamp(rack_max_delta_t[r], (REAL)1.0f, (REAL)60.0f);
      REAL delta_t = clamp(rack_heat_w[r] / (rho * cp * airflow), (REAL)0.0f, max_delta);
      REAL t_in = physics_v1_sample_cold_face_temperature(para, var, i, j, k, size, cold, d);
      REAL target_temp;
      REAL temp;
      REAL source_delta;
      REAL source_limit;
      ...
      if (!(t_in == t_in)) t_in = para->init.T;
      target_temp = t_in + delta_t;
      temp = var[TEMP * size + idx];
      if (!(temp == temp)) temp = t_in;

      if (in_hot) {
        source_limit = clamp(rack_source_delta_limit[r], (REAL)0.05f, (REAL)5.0f);
        source_delta = rack_heat_w[r] * dt / (rho * cp * hot_volume);
        source_delta = clamp(source_delta, (REAL)0.0f, source_limit);
        if (temp < target_temp) {
          temp += min(source_delta, target_temp - temp);
        }
      }

      mix = clamp(dt * max(rack_plume_mixing[r], (REAL)0.0f), (REAL)0.0f, (REAL)0.15f);
      if (mix > (REAL)0.0f) {
        REAL mean_t = physics_v1_neighbor_mean_temperature(para, var, i, j, k, size);
        if (mean_t == mean_t) {
          temp += mix * (mean_t - temp);
        }
      }

      temp = clamp(temp, t_in - (REAL)20.0f, target_temp);
      var[TEMP * size + idx] = temp;
    }
  }
}
