...
int ffd()
{
  ...
  cl_kernel kernel[27] = { NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
    NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL , NULL, NULL, NULL}; //<--- DEFINE HERE THE NUMBER OF KERNELS THAT ARE FOUND IN THE .CL FILE
  PHYSICS_V1_SOURCES physics_v1;

  /*CPU MEMORY ALLOCATIONS*/
  if (ffd_prep(0) != 0) {
    ffd_log("main(): Error in launching the FFD simulation", FFD_ERROR);
  }
  if (load_physics_v1_sources("case.sources", &physics_v1)) {
    sprintf(msg, "main(): loaded physics-v1 sources: racks=%d tiles=%d extractors=%d", physics_v1.rack_count, physics_v1.tile_count, physics_v1.extractor_count);
  ...
  */
  ...
  if (devices_GPU != NULL && numDevices > 0) {
    devices[0] = devices_GPU[0];
  }
  else if (devices_CPU != NULL) {
    devices[0] = devices_CPU[0];
  }
  else {
    ffd_log("main(): no OpenCL GPU or CPU device available", FFD_ERROR);
    return FAILURE;
  }
  ...
  kernel[25] = clCreateKernel(program, "apply_physics_v1_momentum", NULL); //<---- physics-v1 rack/extractor momentum
  kernel[26] = clCreateKernel(program, "apply_physics_v1_thermal", NULL); //<---- physics-v1 rack heat pickup
  ...
  cl_mem rack_body_mobj = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(physics_v1.rack_body_bounds), NULL, &status);
  cl_mem rack_cold_mobj = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(physics_v1.rack_cold_bounds), NULL, &status);
  cl_mem rack_hot_mobj = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(physics_v1.rack_hot_bounds), NULL, &status);
  cl_mem rack_plume_mobj = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(physics_v1.rack_plume_bounds), NULL, &status);
  cl_mem rack_dir_mobj = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(physics_v1.rack_dir), NULL, &status);
  cl_mem rack_heat_mobj = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(physics_v1.rack_heat_w), NULL, &status);
  cl_mem rack_airflow_mobj = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(physics_v1.rack_airflow_m3s), NULL, &status);
  cl_mem rack_hot_volume_mobj = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(physics_v1.rack_hot_volume_m3), NULL, &status);
  cl_mem rack_speed_mobj = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(physics_v1.rack_target_speed), NULL, &status);
  cl_mem rack_momentum_damping_mobj = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(physics_v1.rack_momentum_damping), NULL, &status);
  cl_mem rack_thermal_damping_mobj = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(physics_v1.rack_thermal_damping), NULL, &status);
  cl_mem rack_plume_mixing_mobj = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(physics_v1.rack_plume_mixing), NULL, &status);
  cl_mem rack_source_delta_limit_mobj = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(physics_v1.rack_source_delta_limit), NULL, &status);
  cl_mem rack_max_delta_mobj = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(physics_v1.rack_max_delta_t), NULL, &status);
  cl_mem tile_bounds_mobj = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(physics_v1.tile_bounds), NULL, &status);
  cl_mem tile_loss_mobj = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(physics_v1.tile_loss), NULL, &status);
  cl_mem extractor_bounds_mobj = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(physics_v1.extractor_bounds), NULL, &status);
  cl_mem extractor_dir_mobj = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(physics_v1.extractor_dir), NULL, &status);
  cl_mem extractor_speed_mobj = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(physics_v1.extractor_target_speed), NULL, &status);
  cl_mem extractor_max_speed_mobj = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(physics_v1.extractor_max_speed), NULL, &status);
  cl_mem extractor_damping_mobj = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(physics_v1.extractor_damping), NULL, &status);
  status = clFinish(commandQueue);
  ...
  status = clEnqueueWriteBuffer(commandQueue, rack_body_mobj, CL_TRUE, 0, sizeof(physics_v1.rack_body_bounds), physics_v1.rack_body_bounds, 0, NULL, NULL);
  status = clEnqueueWriteBuffer(commandQueue, rack_cold_mobj, CL_TRUE, 0, sizeof(physics_v1.rack_cold_bounds), physics_v1.rack_cold_bounds, 0, NULL, NULL);
  status = clEnqueueWriteBuffer(commandQueue, rack_hot_mobj, CL_TRUE, 0, sizeof(physics_v1.rack_hot_bounds), physics_v1.rack_hot_bounds, 0, NULL, NULL);
  status = clEnqueueWriteBuffer(commandQueue, rack_plume_mobj, CL_TRUE, 0, sizeof(physics_v1.rack_plume_bounds), physics_v1.rack_plume_bounds, 0, NULL, NULL);
  status = clEnqueueWriteBuffer(commandQueue, rack_dir_mobj, CL_TRUE, 0, sizeof(physics_v1.rack_dir), physics_v1.rack_dir, 0, NULL, NULL);
  status = clEnqueueWriteBuffer(commandQueue, rack_heat_mobj, CL_TRUE, 0, sizeof(physics_v1.rack_heat_w), physics_v1.rack_heat_w, 0, NULL, NULL);
  status = clEnqueueWriteBuffer(commandQueue, rack_airflow_mobj, CL_TRUE, 0, sizeof(physics_v1.rack_airflow_m3s), physics_v1.rack_airflow_m3s, 0, NULL, NULL);
  status = clEnqueueWriteBuffer(commandQueue, rack_hot_volume_mobj, CL_TRUE, 0, sizeof(physics_v1.rack_hot_volume_m3), physics_v1.rack_hot_volume_m3, 0, NULL, NULL);
  status = clEnqueueWriteBuffer(commandQueue, rack_speed_mobj, CL_TRUE, 0, sizeof(physics_v1.rack_target_speed), physics_v1.rack_target_speed, 0, NULL, NULL);
  status = clEnqueueWriteBuffer(commandQueue, rack_momentum_damping_mobj, CL_TRUE, 0, sizeof(physics_v1.rack_momentum_damping), physics_v1.rack_momentum_damping, 0, NULL, NULL);
  status = clEnqueueWriteBuffer(commandQueue, rack_thermal_damping_mobj, CL_TRUE, 0, sizeof(physics_v1.rack_thermal_damping), physics_v1.rack_thermal_damping, 0, NULL, NULL);
  status = clEnqueueWriteBuffer(commandQueue, rack_plume_mixing_mobj, CL_TRUE, 0, sizeof(physics_v1.rack_plume_mixing), physics_v1.rack_plume_mixing, 0, NULL, NULL);
  status = clEnqueueWriteBuffer(commandQueue, rack_source_delta_limit_mobj, CL_TRUE, 0, sizeof(physics_v1.rack_source_delta_limit), physics_v1.rack_source_delta_limit, 0, NULL, NULL);
  status = clEnqueueWriteBuffer(commandQueue, rack_max_delta_mobj, CL_TRUE, 0, sizeof(physics_v1.rack_max_delta_t), physics_v1.rack_max_delta_t, 0, NULL, NULL);
  status = clEnqueueWriteBuffer(commandQueue, tile_bounds_mobj, CL_TRUE, 0, sizeof(physics_v1.tile_bounds), physics_v1.tile_bounds, 0, NULL, NULL);
  status = clEnqueueWriteBuffer(commandQueue, tile_loss_mobj, CL_TRUE, 0, sizeof(physics_v1.tile_loss), physics_v1.tile_loss, 0, NULL, NULL);
  status = clEnqueueWriteBuffer(commandQueue, extractor_bounds_mobj, CL_TRUE, 0, sizeof(physics_v1.extractor_bounds), physics_v1.extractor_bounds, 0, NULL, NULL);
  status = clEnqueueWriteBuffer(commandQueue, extractor_dir_mobj, CL_TRUE, 0, sizeof(physics_v1.extractor_dir), physics_v1.extractor_dir, 0, NULL, NULL);
  status = clEnqueueWriteBuffer(commandQueue, extractor_speed_mobj, CL_TRUE, 0, sizeof(physics_v1.extractor_target_speed), physics_v1.extractor_target_speed, 0, NULL, NULL);
  status = clEnqueueWriteBuffer(commandQueue, extractor_max_speed_mobj, CL_TRUE, 0, sizeof(physics_v1.extractor_max_speed), physics_v1.extractor_max_speed, 0, NULL, NULL);
  status = clEnqueueWriteBuffer(commandQueue, extractor_damping_mobj, CL_TRUE, 0, sizeof(physics_v1.extractor_damping), physics_v1.extractor_damping, 0, NULL, NULL);
  status = clFinish(commandQueue);

  status = clSetKernelArg(kernel[25], 0, sizeof(cl_mem), &para_mobj);
  status |= clSetKernelArg(kernel[25], 1, sizeof(cl_mem), &var_mobj);
  status |= clSetKernelArg(kernel[25], 2, sizeof(int), &physics_v1.rack_count);
  status |= clSetKernelArg(kernel[25], 3, sizeof(cl_mem), &rack_body_mobj);
  status |= clSetKernelArg(kernel[25], 4, sizeof(cl_mem), &rack_cold_mobj);
  status |= clSetKernelArg(kernel[25], 5, sizeof(cl_mem), &rack_hot_mobj);
  status |= clSetKernelArg(kernel[25], 6, sizeof(cl_mem), &rack_dir_mobj);
  status |= clSetKernelArg(kernel[25], 7, sizeof(cl_mem), &rack_speed_mobj);
  status |= clSetKernelArg(kernel[25], 8, sizeof(cl_mem), &rack_momentum_damping_mobj);
  status |= clSetKernelArg(kernel[25], 9, sizeof(int), &physics_v1.extractor_count);
  status |= clSetKernelArg(kernel[25], 10, sizeof(cl_mem), &extractor_bounds_mobj);
  status |= clSetKernelArg(kernel[25], 11, sizeof(cl_mem), &extractor_dir_mobj);
  status |= clSetKernelArg(kernel[25], 12, sizeof(cl_mem), &extractor_speed_mobj);
  status |= clSetKernelArg(kernel[25], 13, sizeof(cl_mem), &extractor_max_speed_mobj);
  status |= clSetKernelArg(kernel[25], 14, sizeof(cl_mem), &extractor_damping_mobj);
  status |= clSetKernelArg(kernel[25], 15, sizeof(int), &physics_v1.tile_count);
  status |= clSetKernelArg(kernel[25], 16, sizeof(cl_mem), &tile_bounds_mobj);
  status |= clSetKernelArg(kernel[25], 17, sizeof(cl_mem), &tile_loss_mobj);
  if (status != CL_SUCCESS) {
    ffd_log("main(): failed to set physics-v1 momentum kernel arguments", FFD_ERROR);
  }

  status = clSetKernelArg(kernel[26], 0, sizeof(cl_mem), &para_mobj);
  status |= clSetKernelArg(kernel[26], 1, sizeof(cl_mem), &var_mobj);
  status |= clSetKernelArg(kernel[26], 2, sizeof(int), &physics_v1.rack_count);
  status |= clSetKernelArg(kernel[26], 3, sizeof(cl_mem), &rack_cold_mobj);
  status |= clSetKernelArg(kernel[26], 4, sizeof(cl_mem), &rack_hot_mobj);
  status |= clSetKernelArg(kernel[26], 5, sizeof(cl_mem), &rack_plume_mobj);
  status |= clSetKernelArg(kernel[26], 6, sizeof(cl_mem), &rack_dir_mobj);
  status |= clSetKernelArg(kernel[26], 7, sizeof(cl_mem), &rack_heat_mobj);
  status |= clSetKernelArg(kernel[26], 8, sizeof(cl_mem), &rack_airflow_mobj);
  status |= clSetKernelArg(kernel[26], 9, sizeof(cl_mem), &rack_hot_volume_mobj);
  status |= clSetKernelArg(kernel[26], 10, sizeof(cl_mem), &rack_max_delta_mobj);
  status |= clSetKernelArg(kernel[26], 11, sizeof(cl_mem), &rack_thermal_damping_mobj);
  status |= clSetKernelArg(kernel[26], 12, sizeof(cl_mem), &rack_plume_mixing_mobj);
  status |= clSetKernelArg(kernel[26], 13, sizeof(cl_mem), &rack_source_delta_limit_mobj);
  if (status != CL_SUCCESS) {
    ffd_log("main(): failed to set physics-v1 thermal kernel arguments", FFD_ERROR);
  }
  ...
  while (NEXT == 1)
  {
    ...
    if (physics_v1.enabled) {
      ...
      //Run kernel
	    ...
	    if (physics_v1.enabled) {
	      /* Projection removes divergence but can weaken the fan/device face
	       * velocities.  Reapply the capped rack/extractor guide before scalar
	       * transport so rack exhaust heat is carried by the modeled fan path. */
  ...
  // Writing the tecplot results for standalone output.
