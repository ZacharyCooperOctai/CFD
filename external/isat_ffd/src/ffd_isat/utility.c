...
int min_distance(PARA_DATA *para, REAL **var, int **BINDEX) {
		...
		REAL *flagp = var[FLAGP], *dist = var[MIN_DISTANCE];
		REAL inf = 1e12;
		REAL candidate, step;
		...
		/*
		| Original code compared every fluid cell with every boundary cell.
		| That is prohibitively expensive for refined native FNB grids.
		| Seed non-inlet/outlet boundary/solid cells at zero, then propagate
		| a structured-grid chamfer distance through fluid cells.
		*/
		FOR_ALL_CELL
				idx = IX(i, j, k);
				if (flagp[idx] >= 0 && flagp[idx] != INLET && flagp[idx] != OUTLET) {
						dist[idx] = 0.0;
				}
				else {
						dist[idx] = inf;
		...
		for(i=1; i<=imax; i++) {
				for(j=1; j<=jmax; j++) {
						for(k=1; k<=kmax; k++) {
								idx = IX(i, j, k);
								if (flagp[idx] >= 0) continue;

								nidx = IX(i-1, j, k);
								step = fabs(x[idx] - x[nidx]);
								candidate = dist[nidx] + step;
								if (candidate < dist[idx]) dist[idx] = candidate;

								nidx = IX(i, j-1, k);
								step = fabs(y[idx] - y[nidx]);
								candidate = dist[nidx] + step;
								if (candidate < dist[idx]) dist[idx] = candidate;

								nidx = IX(i, j, k-1);
								step = fabs(z[idx] - z[nidx]);
								candidate = dist[nidx] + step;
								if (candidate < dist[idx]) dist[idx] = candidate;
						}
				}
