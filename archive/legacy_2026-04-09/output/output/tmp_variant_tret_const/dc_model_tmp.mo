...
model DataCentreFFD
  ...
  parameter Modelica.Units.SI.Temperature T_sup_rated = 14.0 + 273.15
    "Rated CRAC supply air temperature [K]"
    annotation(Dialog(group="CRAC sizing"));

  parameter Modelica.Units.SI.Temperature T_init = 22.0 + 273.15
  ...
  Buildings.ThermalZones.Detailed.CFD roo(
    ...
    // --- Geometry ---
    // Six surfaces: floor, ceiling, 4 walls (all adiabatic)
    // Plenum is included in the FFD domain; Modelica envelope is the whitespace.
    nConExt    = 0,       // no exterior constructions
    nConExtWin = 0,
    nConPar    = 0,
    nConBou    = 0,
    nSurBou    = 6,       // 6 direct surface boundaries (adiabatic)
    // --- Adiabatic direct surface boundaries: floor, ceiling, 4 walls ---
    surBou(
      name    = {"floor", "ceiling", "south_wall", "north_wall",
                 "west_wall", "east_wall"},
      A       = {15.0*25.0, 15.0*25.0,   // floor, ceiling
                 15.0*4.5,  15.0*4.5,    // south wall, north wall
                 25.0*4.5,  25.0*4.5},   // west wall, east wall
...
equation
  ...
  for i in 1:nCRAC loop
    ...
    // Room return port → drain
    connect(roo.ports[nCRAC + i],  retSink.ports[i]);    // return ports 7..12
