...
model TmpTwoSupWithRes
  ...
  Buildings.Fluid.FixedResistances.PressureDrop res1(redeclare package Medium=MediumA, m_flow_nominal=m_flow_rated, dp_nominal=10, allowFlowReversal=false);
  Buildings.Fluid.FixedResistances.PressureDrop res2(redeclare package Medium=MediumA, m_flow_nominal=m_flow_rated, dp_nominal=10, allowFlowReversal=false);
...
equation
  ...
  connect(sup1.ports[1], res1.port_a);
  connect(res1.port_b, roo.ports[1]);
  connect(sup2.ports[1], res2.port_a);
  connect(res2.port_b, roo.ports[2]);
  connect(roo.ports[3], ret.ports[1]);
  connect(roo.ports[4], ret.ports[2]);
end TmpTwoSupWithRes;
