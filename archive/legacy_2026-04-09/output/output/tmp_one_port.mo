...
model TmpOnePort
  ...
  Buildings.ThermalZones.Detailed.CFD roo(
    ...
    hRoo=4.5,
    sensorName={""},
    portName={"crac_1_supply","crac_1_return"},
    nPorts=2);
  ...
  Buildings.Fluid.Sources.MassFlowSource_T sup(redeclare package Medium=MediumA, nPorts=1, use_m_flow_in=true, use_T_in=true, m_flow=4.5*1.204, T=287.15);
  Buildings.Fluid.Sources.Boundary_pT retSink(redeclare package Medium=MediumA, nPorts=1);
  Modelica.Blocks.Sources.Constant fan(k=1.0);
  Modelica.Blocks.Sources.Constant Tsup(k=287.15);
equation
  ...
  connect(fan.y, sup.m_flow_in);
  connect(Tsup.y, sup.T_in);
  connect(sup.ports[1], roo.ports[1]);
  connect(roo.ports[2], retSink.ports[1]);
end TmpOnePort;
